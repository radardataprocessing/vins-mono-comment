...
class IntegrationBase
{
  public:
...

    {
        ...
        noise.block<3, 3>(0, 0) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity();// corresponds to na(k)
        noise.block<3, 3>(3, 3) =  (GYR_N * GYR_N) * Eigen::Matrix3d::Identity();// corresponds to nw(k)
        noise.block<3, 3>(6, 6) =  (ACC_N * ACC_N) * Eigen::Matrix3d::Identity();// corresponds to na(k+1)
        noise.block<3, 3>(9, 9) =  (GYR_N * GYR_N) * Eigen::Matrix3d::Identity();// corresponds to nw(k+1)
        noise.block<3, 3>(12, 12) =  (ACC_W * ACC_W) * Eigen::Matrix3d::Identity();// corresponds to nba
        noise.block<3, 3>(15, 15) =  (GYR_W * GYR_W) * Eigen::Matrix3d::Identity();// corresponds to nbw
    }
    ...
    /*
     * I think the function repropagate means to propagate imu measurements from the first element in dt_buf, acc_buf and gyr_buf
     */
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        /*
         * 15 in this function means: 1.delta_position 2.delta_orientation 3.delta_velocity 4.delta_ba 5.delta_bw
         * 18 in this function means: 1. na(k) 2.nw(k) 3.na(k+1) 4.nw(k+1) 5.nba 6.nbw
         */
        // here, the integration did not take gravity into consideration
        Vector3d un_acc_0 = delta_q * (_acc_0 - linearized_ba);
        Vector3d un_gyr = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
        result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * _dt / 2, un_gyr(1) * _dt / 2, un_gyr(2) * _dt / 2);
        Vector3d un_acc_1 = result_delta_q * (_acc_1 - linearized_ba);
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);
        result_delta_p = delta_p + delta_v * _dt + 0.5 * un_acc * _dt * _dt;//the result_delta_p did not consider the velocity, only considered the delta velocity
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        delta_q.normalize();// normalize the quaternion
        sum_dt += dt;
        acc_0 = acc_1;//set the acc and gyr value of last time tobe the current value
        gyr_0 = gyr_1;  
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        // use dba,dbg and the jacobian matrix to correct delta_p, delta_q and delta_v
        Eigen::Quaterniond corrected_delta_q = delta_q * Utility::deltaQ(dq_dbg * dbg);
        Eigen::Vector3d corrected_delta_v = delta_v + dv_dba * dba + dv_dbg * dbg;
        Eigen::Vector3d corrected_delta_p = delta_p + dp_dba * dba + dp_dbg * dbg;

        // both the delta_p and delta_v did not take gravity into gravity into consideration, so here we add gravity into that question
